...
from auto_ban import auto_ban, AUTOBAN_CODES_G1, AUTOBAN_CODES_G2, GLOBAL_AUTOBAN_CODES, AUTOBAN_ENFORCE, AUTOBAN_G2_READY
...
def _ipinfo_block(geo):
    """IPinfo flags block for extra_payload — panel audit surfaces via badges (restored 2026-07-10)."""
    if geo is None:
        return None
    return {
        ...
        "city": geo.city or "",
        "asn": geo.asn or 0,
        "asn_type": geo.asn_type or "",
        "vpn": bool(getattr(geo, "vpn", False)),
        "proxy": bool(getattr(geo, "proxy", False)),
        "res_proxy": bool(getattr(geo, "res_proxy", False)),
        "tor": bool(getattr(geo, "tor", False)),
        "relay": bool(getattr(geo, "relay", False)),
        "hosting": bool(getattr(geo, "hosting", False)),
        "mobile": bool(getattr(geo, "is_mobile", False)),
        "anonymous_name": getattr(geo, "anonymous_name", "") or "",
    }
...
async def _mark_instance_burnt(instance_id: str, package_name: str, reason: str):
    ...
    2026-07-09 (max-conversion mode): no-op. Клиентское решение — не морозить
    возвраты юзера. Механизм оставлен, чтобы можно было вернуть одной строкой.
    """
    return  # max-conversion mode: не пишем ключ
    if not instance_id:
        return
    try:
...
async def _is_instance_burnt(instance_id: str, package_name: str) -> str:
    ...
    2026-07-09 (max-conversion mode): всегда возвращает "" — даже если
    старые ключи ещё живут в Redis (24h TTL), мы их игнорируем.
    """
    return ""  # max-conversion mode: не блокируем даже existing burnt
    if not instance_id:
        return ""
    try:
        r = await request_logger._get_redis()
...
async def _is_first_init(instance_id: str, package_name: str) -> bool:
    ...
    2026-07-09 (max-conversion mode): всегда возвращает False → UNEVALUATED
    для новых устройств больше не блокирует, реальные юзеры не режутся.
    """
    return False  # max-conversion mode: не gate'им UNEVALUATED
    if not instance_id:
...
async def _asn_rotation_check(instance_id: str, asn: int):
    ...
    2026-07-09 (max-conversion mode): no-op. ASN rotation больше не морозит.
    """
    return 0, []  # max-conversion mode: не блокируем
...
async def _verify_integrity(token: str, package_name: str, mode: str = 'lenient',
    ...
    # 2026-07-10 P7: per-app skip_nonce_replay флаг для v3 SDK прил (Stake) —
    # SDK v3 Classic PI кэширует токен и переиспользует → ложно ловится как replay.
    # Bot-pool через reuse защищён velocity_check.
    skip_nr = app is not None and getattr(app, 'skip_nonce_replay', False)
    if verdict.nonce and not skip_nr:
        try:
            r = await request_logger._get_redis()
            nonce_key = f"pi_nonce_seen:{verdict.nonce}"
            added = await r.set(nonce_key, "1", nx=True, ex=15)  # 2026-07-10: 120→15 anti-legit-cutoff
            if not added:
...
async def _velocity_check(ip: str, instance_id: str):
    ...
    try:
        ...
        if instance_id:
            ...
            # 2026-07-09: max-conversion — порог 12 → 100 (не бракуем нормальный usage pattern)
            if n > 100:
                score += AUTOBAN_SCORE
                details.append(ScoringDetail(
        ...
        if ip and instance_id:
            ...
            # 2026-07-09: max-conversion — порог 15 → 100 (CGNAT дружелюбно)
            if await r.scard(ks) > 100:
                score += AUTOBAN_SCORE
                details.append(ScoringDetail(
                    check="velocity_subnet", points=AUTOBAN_SCORE,
                    reason=f"Velocity: >100 разных install из подсети {subnet} за 15мин — прокси-пул",
                ))
        ...
        if ip:
            ...
            # 2026-07-09: max-conversion — порог 60 → 300 (CGNAT-friendly)
            if ni > 300:
                score += AUTOBAN_SCORE
                details.append(ScoringDetail(
...
async def _resolve(request: Request, proxy_key: str, _trace=None):
    """_trace=None → обычный flow (production).
 _trace={} → trace mode:
    ...
    # 2026-07-10: prefer X-App-Id from SDK if valid pkg in apps.json.
    # Fix: 2 прилы могут иметь один proxy_key (Snai: muslimazkarpro + omniconvert) →
    # без этого все клика идут под тем что записан в proxy_keys.json.
    # 2026-07-15 SECURITY: guard cross-app leak — X-App-Id должен быть в allowed_packages
    # списке для этого proxy_key. Раньше любая прила из apps.json проходила → 1 leaked
    # proxy_key = master key ко всем прилам (client demonstrated 2026-07-15).
    x_app_id = request.headers.get("x-app-id", "").strip() or request.query_params.get("app_id", "").strip()
    allowed = key_data.get("allowed_packages") or [key_data["package_name"]]
    if x_app_id:
        if x_app_id not in allowed:
            logger.warning(
                f"xappid_cross_leak_blocked pk={proxy_key[:12]}... "
                f"x_app_id={x_app_id} allowed={allowed} "
    ...
    # 2026-07-10 X-Real-User-IP custom header — CF не трогает custom headers,
    # значит настоящий user IP от mini-server nginx доходит до fastapi
    real_user_ip = request.headers.get("x-real-user-ip", "").strip()
    ...
    # 2026-07-15 skip_internal_ip: private/docker/localhost hits не должны попадать в scoring
    # (health checks / cron probes / panel-side fetch без X-Forwarded-For → ipinfo не резолвит private
    # → создавал geo_unknown hard_kill записи в БД).
    # Приlы жалоба клиента 15.07: 490 rejections/7d от 172.18.0.1 (docker bridge). Root fix.
    _priv_prefixes = ("127.", "10.", "172.16.", "172.17.", "172.18.", "172.19.",
                      "172.20.", "172.21.", "172.22.", "172.23.", "172.24.",
                      "172.25.", "172.26.", "172.27.", "172.28.", "172.29.",
                      "172.30.", "172.31.", "192.168.", "::1", "fc00:", "fd00:", "0.0.0.0")
    if ip.startswith(_priv_prefixes):
        try:
            _pkg_log = app.package_name if app else "?"
        except Exception:
            _pkg_log = "?"
        logger.info(
            f"skip_internal_ip ip={ip} pkg={_pkg_log} "
            f"host={request.headers.get('host','')} — health/probe, no logging"
        )
    ...
    if config_store.is_debug(ip, instance_id):
        ...
        # 2026-07-10 P7: restore clickid+geo для Keitaro атрибуции
    ...
    # PI токен берём заранее (нужен и для autoban-log, и дальше для scoring).
    ...
    # F-autoban (2026-06-23): TOP-LAYER negative cache. Если (ip, pkg) уже в banlist —
    # instant white БЕЗ всех остальных проверок (IPQS / IPinfo / PI verify / scoring).
    # Экономит IPQS quota + IPinfo + PI verify + DB write.
    # Стоит ПОСЛЕ debug_bypass (чтобы можно было override через debug list) и ДО hard-kills.
    # Логика is_banned: pkg-specific сначала, потом global. Negative cache (mark_grey 1h)
    # защищает legit IPs которые недавно были grey.
    if AUTOBAN_ENFORCE:
        is_banned, ban_code = await auto_ban.is_banned(ip, package_name)
        if is_banned:
            safe_url = app.safe_url if app else config_store.offers.safeUrl
            _pstep(_trace, "autoban", "fired", f"Auto-ban active: {ban_code}")
            if _trace is not None:
                from classifier import classify_actor
                _trace["verdict"] = "white"
                _trace["hard_kill"] = {"code": "autoban_hit", "reason": f"Auto-ban active (original code: {ban_code})"}
                _trace["score_buckets"] = {"base": AUTOBAN_SCORE, "pi": 0, "vel": 0, "extra": 0, "total": AUTOBAN_SCORE}
                _trace["all_details"] = [{"check": "autoban_hit", "points": AUTOBAN_SCORE,
                                         "reason": f"IP в banlist (original: {ban_code})"}]
                _trace["rejection_code"] = "autoban_hit"
                _trace["threshold"] = config_store.engine.scoreThreshold
                _trace["classification"] = classify_actor(_trace)
                return package_name, "white", safe_url

            # Production path: minimal log + return safe URL
            await request_logger.log(
                ip=ip,
                result=ScoringResult(
                    score=AUTOBAN_SCORE, verdict="white", rejectionCode="autoban_hit",
                    details=[ScoringDetail(
                        check="autoban_hit", points=AUTOBAN_SCORE,
                        reason=f"IP в auto-banlist (original: {ban_code})")],
                ),
                user_agent=user_agent, accept_language=accept_language,
                country="", country_code="", city="",
                headers={"x-app-id": package_name, "source": "init",
                         "x-proxy-key": proxy_key[:8] + "...", "user-agent": user_agent,
                         "x-instance-id": instance_id, "has-integrity": "yes" if integrity_token else "no",
                         "asn": "", "hard-kill": "autoban_hit", "ban-code": ban_code},
                js_metrics=None,
                extra_payload={
                    "scoreBuckets": {"base": AUTOBAN_SCORE, "pi": 0, "vel": 0, "extra": 0, "total": AUTOBAN_SCORE},
                    "hardKill": {"code": "autoban_hit", "reason": f"banned: {ban_code}"},
                },
            )
            logger.info(f"[{ip}] init: pkg={package_name} AUTOBAN-HIT code={ban_code} → white")
            return package_name, "white", safe_url
    ...
    if hard_kill_reason:
        ...
        await request_logger.log(
            ...
            extra_payload={
                ...
                "ipinfo": _ipinfo_block(geo),
            },
        )
        logger.info(f"[{ip}] init: pkg={package_name} HARD-KILL asn={asn} → white {reject_code}")

        # F-autoban (2026-06-23): emit hard-kill в banlist чтобы следующий /init с этого
        # (ip, pkg) не прогонять через все слои. Только для AUTOBAN_CODES_G1 (всегда)
        # и G2 (если AUTOBAN_G2_READY=true). Shadow mode (AUTOBAN_ENFORCE=false) →
        # запишет в DB с would_ban=true, но НЕ в Redis (не enforces). Async fire-and-forget.
        try:
            if reject_code in AUTOBAN_CODES_G1 or (reject_code in AUTOBAN_CODES_G2 and AUTOBAN_G2_READY):
                pkg_key = None if reject_code in GLOBAL_AUTOBAN_CODES else package_name
                await auto_ban.emit(
                    ip=ip, package_name=pkg_key, code=reject_code,
                    reason=reason_text[:500],
                    origin_request_id=None,  # request_logger generates its own id, skip
                    classification=(classify_actor(_hk_trace) or {}).get("class"),
                )
        except Exception as _e_auto:
            logger.warning(f"AutoBan emit failed (hard-kill): {_e_auto}")
    ...
    if not rejection and pi_score >= AUTOBAN_SCORE:
        # 2026-07-15 device_compromised DISABLED (client) — оставили только integrity_missing
        if not integrity_token:
            rejection = "integrity_missing"
        # else: device_compromised removed
    ...
    # F-autoban (2026-06-23): emit hook на scoring path:
    # - verdict=white + rejection_code в AUTOBAN_CODES → add to banlist
    # - verdict=grey → mark_grey (negative cache 1h, защита от ошибочного ban legit IP)
    try:
        if verdict == "white" and rejection and (
            rejection in AUTOBAN_CODES_G1 or (rejection in AUTOBAN_CODES_G2 and AUTOBAN_G2_READY)
        ):
            pkg_key = None if rejection in GLOBAL_AUTOBAN_CODES else package_name
            await auto_ban.emit(
                ip=ip, package_name=pkg_key, code=rejection,
                reason=f"scoring: score={total_score}, threshold={threshold}",
                classification=(classification or {}).get("class") if isinstance(classification, dict) else None,
            )
        elif verdict == "grey":
            await auto_ban.mark_grey(ip, package_name)
    except Exception as _e_auto:
        logger.warning(f"AutoBan emit failed (scoring): {_e_auto}")
...
async def init_resolve(request: Request):
    ...
    # 2026-07-09: добавлен POST — v4 SDK шлёт POST, направляемый nginx-ом mini-серверов
    # на этот handler; без POST-роута nginx receives 405 и не может проксировать v4 flow.
