...
class IntegrityVerdict:
    def __init__(self, data):
        ...
        # v4 Standard Integrity API кладёт requestHash вместо nonce — покрываем оба для replay-защиты
        self.nonce = req.get("nonce", "") or req.get("requestHash", "")
...
class PlayIntegrityClient:
    ...
    def _load_all_keys(self) -> dict:
        ...
        for key_file in sorted(CONFIG_DIR.glob("gcp-key*.json")):
            try:
                ...
                # 2026-07-07: bounded HTTP (10s) — decode рвётся при сетевом зависании,
                # не висит до OS TCP timeout, не течёт executor-поток.
                authed = AuthorizedHttp(credentials, http=httplib2.Http(timeout=10))
                services[project_id] = build("playintegrity", "v1", http=authed, cache_discovery=False)
    ...
    async def verify_token(self, integrity_token: str, package_name: str = "") -> Optional[IntegrityVerdict]:
        ...
        try:
            ...
            # 2026-07-07: таймаут на Google PI decode. Без него .execute() мог висеть
            # (Google API тупит на UNRECOGNIZED/sandbox токенах) → КЛО зависал → middleware
            # ждал → SDK ловил 15s SocketTimeout → openNativeGame() (заглушка). 8s < SDK 15s.
            try:
                result = await asyncio.wait_for(loop.run_in_executor(None, _decode), timeout=10.0)
            except asyncio.TimeoutError:
                logger.error(f"Play Integrity decode TIMEOUT (>10s) [{resolved_pkg}] — treating as no-PI")
                return None
